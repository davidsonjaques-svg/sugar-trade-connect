...
def _render_order_detail(order, customer):
    ...
    with col2:
        st.markdown(f"""
        <div class="card">
            <div style="font-weight:700; font-size:14px; color:#1B4332; margin-bottom:12px;
                        text-transform:uppercase; letter-spacing:0.05em;">Pricing Breakdown</div>
            <table style="width:100%; font-size:14px; border-collapse:collapse;">
                <tr>
                    <td style="color:#6B7280; padding:5px 0;">Product Subtotal</td>
                    <td style="font-weight:600; text-align:right;">{format_zar(order['subtotal'])}</td>
                </tr>
                <tr>
                    <td style="color:#6B7280; padding:5px 0;">
                        Platform Fee ({int(order.get('platform_fee_rate', 0.015)*100)}%)
                    </td>
                    <td style="font-weight:600; text-align:right; color:#D4A017;">
                        {format_zar(order.get('platform_fee', order['subtotal'] * 0.015))}
                    </td>
                </tr>
                <tr style="border-top:1px solid #E5E0D5;">
                    <td style="color:#6B7280; padding:5px 0;">Subtotal before VAT</td>
                    <td style="font-weight:600; text-align:right;">
                        {format_zar(order.get('subtotal_after_fees', order['subtotal'] * 1.015))}
                    </td>
                </tr>
                <tr>
                    <td style="color:#6B7280; padding:5px 0;">VAT ({int(order['vat_rate']*100)}%)</td>
                    <td style="font-weight:600; text-align:right; color:#D4A017;">{format_zar(order['vat_amount'])}</td>
                </tr>
                <tr style="border-top:2px solid #1B4332;">
                    <td style="color:#1B4332; font-weight:700; font-size:15px; padding:8px 0 4px 0;">TOTAL DUE</td>
                    <td style="font-weight:700; font-size:18px; color:#1B4332; text-align:right; padding-top:8px;">
                ...
                <tr style="border-top:1px dashed #E5E0D5;">
                    <td style="color:#9CA3AF; font-size:12px; padding:8px 0 2px 0;">
                        Gateway Fee (est. {int(order.get('gateway_fee_rate', 0.029)*100)}% — absorbed)
                    </td>
                    <td style="color:#9CA3AF; font-size:12px; text-align:right; padding-top:8px;">
                        ({format_zar(order.get('gateway_fee', order['total_amount'] * 0.029))})
                    </td>
                </tr>
                <tr>
                    <td style="color:#6B7280; font-size:12px; padding:2px 0;">Net to Sugar Trade Connect</td>
                    <td style="color:#1B4332; font-size:13px; font-weight:700; text-align:right;">
                        {format_zar(order.get('net_to_trader', order['total_amount'] * 0.971))}
                    </td>
                </tr>
            </table>
        </div>
