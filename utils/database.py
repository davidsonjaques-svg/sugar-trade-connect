...
def init_db():
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS orders (
            ...
            platform_fee_rate REAL NOT NULL DEFAULT 0.015,
            platform_fee REAL NOT NULL DEFAULT 0.0,
            subtotal_after_fees REAL NOT NULL DEFAULT 0.0,
            vat_rate REAL NOT NULL DEFAULT 0.15,
            vat_amount REAL NOT NULL,
            total_amount REAL NOT NULL,
            gateway_fee_rate REAL NOT NULL DEFAULT 0.029,
            gateway_fee REAL NOT NULL DEFAULT 0.0,
            net_to_trader REAL NOT NULL DEFAULT 0.0,
...
def create_order(customer_id, product_type, grade, quantity_kg, unit_price,
                 ...
                 vat_rate=0.15, platform_fee_rate=0.015, gateway_fee_rate=0.029):
    subtotal = quantity_kg * unit_price
    platform_fee = subtotal * platform_fee_rate
    subtotal_after_fees = subtotal + platform_fee
    vat_amount = subtotal_after_fees * vat_rate
    total_amount = subtotal_after_fees + vat_amount
    gateway_fee = total_amount * gateway_fee_rate
    net_to_trader = total_amount - gateway_fee
    ...
    c.execute("""
        INSERT INTO orders (order_ref, customer_id, product_type, grade, quantity_kg,
            unit_price, subtotal, platform_fee_rate, platform_fee, subtotal_after_fees,
            vat_rate, vat_amount, total_amount, gateway_fee_rate, gateway_fee, net_to_trader,
            delivery_address, delivery_date, special_instructions)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (order_ref, customer_id, product_type, grade, quantity_kg, unit_price,
          subtotal, platform_fee_rate, platform_fee, subtotal_after_fees,
          vat_rate, vat_amount, total_amount, gateway_fee_rate, gateway_fee, net_to_trader,
          delivery_address, delivery_date, special_instructions))
